...
class Simulator_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scaled_time  = (non_relative_time_tensor-min_time)/(max_time-min_time)
        initial_voltage = voltage[0]
        initial_temperature = temperature[0]
        initial_remaining_cycle = remaining_cycle[0]
        initial_state = torch.stack((initial_remaining_cycle,initial_voltage, initial_temperature)).squeeze()
        #print(initial_state.shape,"$$$$ Stacked tensor")

        #print(initial_voltage.shape,initial_temperature.shape,initial_remaining_cycle.shape,"#############")
        action_taken = current[1:]
        #print(action_taken.shape,"## Action taken shape")

        new_voltage_characteristics = voltage[1:]
        new_temperature_characteristics = temperature[1:]
        final_characteristics = torch.cat((new_voltage_characteristics, new_temperature_characteristics), dim=1)

        #print(new_voltage_characteristics.shape,new_temperature_characteristics.shape,"#############",final_characteristics.shape)

        #print(jd)


        return initial_state, action_taken, final_characteristics
...
simulator_dataset = Simulator_dataset(
    ...
    remaining_cycles,
    non_relative_time_stitched,
    sequence_length=151 # 151 output predicted by the states
)

## We define the train,validation and test ration below
total_dataset_size = len(simulator_dataset)
...
sim_train_dataset = torch.utils.data.Subset(simulator_dataset, train_indices.tolist())
sim_val_dataset = torch.utils.data.Subset(simulator_dataset, val_indices.tolist())
sim_test_dataset = torch.utils.data.Subset(simulator_dataset, test_indices.tolist())
